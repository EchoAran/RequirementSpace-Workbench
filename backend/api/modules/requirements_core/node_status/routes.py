...
_NODE_KIND_LABEL = {
    ...
    "business_object_attribute": "业务对象属性",
    "flow": "流程",
    "flow_step": "流程步骤",
}
...
async def _apply_status_update(
    ...
    elif model_class is BusinessObjectAttributeModel:
        stmt = (
            select(BusinessObjectAttributeModel)
            .join(
                BusinessObjectModel,
                BusinessObjectAttributeModel.business_object_id == BusinessObjectModel.id,
            )
            .where(
                BusinessObjectModel.project_id == project_id,
                BusinessObjectAttributeModel.id == node_id,
            )
        )
    elif model_class is FlowStepModel:
        stmt = (
            select(FlowStepModel)
            .join(FlowModel, FlowStepModel.flow_id == FlowModel.id)
            .where(
                FlowModel.project_id == project_id,
                FlowStepModel.id == node_id,
            )
        )
    else:
