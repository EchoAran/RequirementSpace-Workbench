"""
Phase 2: Project Creation Choice Group Service

Use GenerativeDraftModel to store onboarding choice groups (since
choice_groups.project_id cannot be null during this phase). After the
user accepts a candidate, the project is created via the existing
ProjectCreationService._persist_project_creation_draft(), and the old
generative draft is deleted.

API endpoints (separate from /api/generation_choice_groups because
they operate on drafts, not on real choice_groups table):

  POST   /api/project_creation_choice_groups
  GET    /api/project_creation_choice_groups/{group_id}
  GET    /api/project_creation_choice_groups?status=open
  POST   /api/project_creation_choice_groups/{group_id}/choices/{choice_id}/accept
  POST   /api/project_creation_choice_groups/{group_id}/discard
"""
import asyncio
import hashlib
import json
...
@register_adapter("project_creation")
