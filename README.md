...
根目录 `requirements.txt` 只声明项目直接需要的 Python 运行依赖，传递依赖由 pip 自动解析。

主要依赖包括：

- `fastapi`、`uvicorn[standard]`：后端 API 和开发服务器
- `SQLAlchemy`、`aiosqlite`、`alembic`：SQLite 持久化和迁移
- `pydantic`：接口数据模型
- `httpx`、`python-dotenv`：LLM HTTP 调用和环境变量读取
- `openai`：集成 skill 包的直接 OpenAI SDK 依赖
- `dataclasses-json`、`inflection`：旧版需求 IR 模型序列化
- `matplotlib`：Kano 图表渲染
- `tzdata`：Windows 等平台上的时区数据
