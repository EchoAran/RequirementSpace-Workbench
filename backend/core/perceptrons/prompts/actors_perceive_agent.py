...
# 核心定义
1. 参与者：与系统进行交互、有独立且不可替代的利益诉求和不同权限/职责边界的外部实体（人）
2. 核心参与者：没有该参与者，系统的核心功能就无法正常运行或失去存在意义
...
5. 禁止过度拆分原则：同一个参与者在系统中如果只有一套权限和利益诉求，无论其执行多少种操作，都只能作为一个参与者。
...
## 错误反例（绝对禁止输出）
{
    "perception_description": "需要补充：便签分享者。原因：用户可能会分享便签给他人。"
}
### 错误原因
    - 用户需求中没有提到"分享功能"，分享者属于边缘潜在使用者，不是核心参与者
"""
