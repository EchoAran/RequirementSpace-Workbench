...
if __name__ == "__main__":
    ...
    test_perception_slot = PerceptionSlot(
        perceptionSlotId = 1,
        perceptionKind = PerceptionKindType.ACTOR,
    ...
    async def main():
         await actors_filler.fill(
            ActorsFillerInput(user_requirements, test_actors, test_perception_slot)
        )
    asyncio.run(main())

    actors_filler_result = """
{
  "actors": [
    {
      "actor_name": "本地音乐库维护者",
      "actor_description": "本地音乐库维护者是指在需要维护和整理本地音乐文件库的场景下，与本地音乐库管理功能发生交互，并可执行选择或配置扫描目录、手动触发扫描或刷新、排除特定目录、处理重复歌曲、修正或编辑基础标签信息（如曲名、歌手、专辑、封面）、移除无效文件记录以及维护音乐库目录结构等操作的用户角色。"
    }
  ]
}
"""
