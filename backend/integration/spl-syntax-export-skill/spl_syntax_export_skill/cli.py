...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="spl-syntax-export-skill",
        description="Deterministic code-based syntax wrapper export to SPL.",
    )
    parser.add_argument(
        "--input-json",
        required=True,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        skill = SplSyntaxExportSkill()
        result = skill.export(payload)
        
        if args.output_file:
            out_path = Path(args.output_file)
            out_path.write_text(result["spl_text"], encoding="utf-8")
            print(f"Exported SPL syntax to {out_path}")
        else:
            print(json.dumps(result, ensure_ascii=False, indent=2))
        return 0
    except Exception as exc:
        print(f"spl-syntax-export-skill failed: {exc}", file=sys.stderr)
        return 1
