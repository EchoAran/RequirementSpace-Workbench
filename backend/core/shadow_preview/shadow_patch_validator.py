...
class ShadowPatchValidator:
    ...
    def validate_patch(patch: dict, base_snapshot: dict, project_id: str) -> bool:
        ...
        if base_snapshot.get("project_id") != project_id:
            raise ValueError(
                f"Project ID mismatch: base_snapshot is for project {base_snapshot.get('project_id')}, "
        ...
        # Ensure that flow step positions are sequential and consecutive starting from 1 for each flow_ref
        flow_steps_by_flow: dict[str, list[dict]] = {}
        for st in steps_added:
            ...
            # actorAction must have at least one actor
