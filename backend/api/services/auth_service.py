...
from backend.api.schemas.auth_schema import RegisterRequest, LoginRequest
...
class AuthService:
    ...
    async def register_user(request: RegisterRequest, session: AsyncSession) -> UserModel:
        ...
        if request.invite_code is not None:
            if not ADMIN_INVITE_CODE_HASH:
                raise HTTPException(status_code=400, detail="invalid_invite_code")
            if not verify_password(request.invite_code, ADMIN_INVITE_CODE_HASH):
                raise HTTPException(status_code=400, detail="invalid_invite_code")
