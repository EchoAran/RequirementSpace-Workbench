            ...
            <div className="space-y-6">
              <div className="p-4 rounded-2xl bg-indigo-50/50 border border-indigo-100 text-indigo-950 space-y-3">
                ...
                <p className="text-xs leading-relaxed text-slate-600">
                  由于您是以<strong>管理员</strong>身份登录的，系统将直接默认读取服务器环境配置文件（`.env`）中配置的共享大语言模型 API 资源，您无需手动在此配置个人 API 信息。
                </p>
              </div>
          ...
          {testResult && (
            ...
            >
              ...
              <div className="space-y-1">
                <span className="font-bold">{testResult.success ? '测试通过' : '测试失败'}</span>
                <p className="leading-relaxed text-slate-600">{testResult.message}</p>
              </div>
            </div>
          )}
        </section>
